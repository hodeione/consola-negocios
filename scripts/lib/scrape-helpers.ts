/**
 * Piezas compartidas entre los scripts de scraping local (`local-scrape.ts`,
 * manual, y `local-scrape-coverage.ts`, cobertura automática) — mismo motor
 * de recogida/detalle/enriquecido y mismo formato de .xlsx que "Exportar
 * Excel" de la app, para que cualquier fichero generado se pueda subir tal
 * cual con "Importar Excel".
 */
...
export async function collectAllLinks(keyword: string, zone: string, language: string, maxResults: number): Promise<string[]> {
  let links: string[] = [];
  for (let i = 0; i < 20; i++) {
...
export async function detailAllPlaces(links: string[], language: string): Promise<PlaceDetails[]> {
